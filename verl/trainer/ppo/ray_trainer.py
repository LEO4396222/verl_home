...
from verl.trainer.ppo.core_algos import AdvantageEstimator, _maybe_clip_advantages, agg_loss
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.sigmoid_entropy_target = None
    ...
    def _filter_all_correct_wrong_samples(
        self, reward_tensor: torch.Tensor, response_mask: torch.Tensor, group_ids: Optional[np.ndarray]
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Filter out prompts whose all responses are correct or all wrong within the current batch.

        The filtered samples are skipped only for this batch (mask掉), 数据在后续 batch 仍可被采样。
        """
        if not self.config.trainer.get("filter_all_correct_wrong", False):
            return reward_tensor, response_mask
        ...
        # 每个响应的 outcome 奖励
        response_rewards = (reward_tensor * response_mask).sum(dim=-1)
        valid_rows = mask.any(dim=1)
        valid_np = valid_rows.detach().cpu().numpy()

        gid_array = np.asarray(group_ids)
        to_filter = torch.zeros_like(valid_rows, dtype=torch.bool, device=device)
        for gid in np.unique(gid_array):
            idx = np.where(gid_array == gid)[0]
            idx = [i for i in idx if valid_np[i]]
            if not idx:
                continue
            idx_tensor = torch.as_tensor(idx, device=device)
            rewards = response_rewards[idx_tensor]
            if torch.all(rewards > 0) or torch.all(rewards < 0):
                to_filter[idx_tensor] = True

        if not torch.any(to_filter):
            return reward_tensor, response_mask

        filtered_reward = reward_tensor.clone()
        filtered_mask = response_mask.clone()
        filtered_reward[to_filter] = 0
        filtered_mask[to_filter] = 0

        return filtered_reward, filtered_mask
    ...
    def _log_rollout_data(
        ...
        *,
        enable_token_ranks: bool = True,
        enable_token_pairs: bool = False,
        token_pair_dir: str | None = None,
        token_pair_log_freq: int = 10,
    ):
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            if reward_extra_infos_dict:
                for k in reward_extra_infos_dict.keys():
                    if k in batch.non_tensor_batch:
                        vals = batch.non_tensor_batch[k]
                        try:
                            vals = vals.tolist()
                        except AttributeError:
                            pass
                        reward_extra_infos_to_dump[k] = vals
                    else:
                        reward_extra_infos_to_dump[k] = reward_extra_infos_dict[k]
            ...
            def _rank_pairs(prob: torch.Tensor, adv: torch.Tensor, m: torch.Tensor):
                """返回掩码子集的 rank 比例(0=最低，1=最高)，用于精细绘制 2D 分布。"""
                prob_vals = prob.masked_select(m)
                adv_vals = adv.masked_select(m)
                if prob_vals.numel() == 0 or adv_vals.numel() == 0:
                    return None
                n = prob_vals.numel()
                denom = max(n - 1, 1)
                # 概率低->rank 低；优势小->rank 低
                prob_order = torch.argsort(prob_vals, descending=False)
                adv_order = torch.argsort(adv_vals, descending=False)
                prob_rank = torch.empty_like(prob_vals, dtype=torch.float)
                adv_rank = torch.empty_like(adv_vals, dtype=torch.float)
                prob_rank[prob_order] = torch.arange(n, device=prob_vals.device, dtype=torch.float)
                adv_rank[adv_order] = torch.arange(n, device=adv_vals.device, dtype=torch.float)
                return [
                    {
                        "prob_rank": float(p / denom),
                        "adv_rank": float(a / denom),
                    }
                    for p, a in zip(prob_rank.tolist(), adv_rank.tolist(), strict=True)
                ]
            ...
            if advantages is not None and response_mask is not None:
                with torch.no_grad():
                    ...
                    if (enable_token_ranks or enable_token_pairs) and old_log_probs is not None:
                        token_prob = torch.exp(old_log_probs).to(dtype=advantages.dtype)
                    if enable_token_ranks and token_prob is not None:
                        pos_mask = (response_mask > 0) & (advantages > 0)
                        neg_mask = (response_mask > 0) & (advantages < 0)
                        pos_pairs = _rank_pairs(token_prob, advantages, pos_mask)
                        neg_pairs = _rank_pairs(token_prob, advantages, neg_mask)
            ...
            # 追加一条 token 级分位数记录，包含当前 batch 全部 token（不重复）。
            if enable_token_ranks and (pos_pairs is not None or neg_pairs is not None):
                filename = os.path.join(rollout_data_dir, f"{self.global_steps}.jsonl")
                token_rank_entry = {
                    "type": "token_ranks",
                    "step": self.global_steps,
                    "adv_prob_ranks_pos": pos_pairs,
                    "adv_prob_ranks_neg": neg_pairs,
                }
                with open(filename, "a") as f:
                    f.write(json.dumps(token_rank_entry, ensure_ascii=False) + "\n")

            # 追加 token 概率 + 裁剪前/后优势三元组（独立目录，按频率触发）
            if (
                enable_token_pairs
                and token_pair_dir
                and token_prob is not None
                and clipped_adv is not None
                and advantages is not None
            ):
                valid_mask = response_mask.to(dtype=torch.bool) if response_mask is not None else None
                if valid_mask is not None and torch.any(valid_mask):
                    probs_flat = token_prob.masked_select(valid_mask)
                    adv_flat = advantages.masked_select(valid_mask)
                    adv_clipped_flat = clipped_adv.masked_select(valid_mask)
                    pairs = [
                        {"prob": float(p), "adv_raw": float(a_raw), "adv_clipped": float(a_clip)}
                        for p, a_raw, a_clip in zip(
                            probs_flat.tolist(), adv_flat.tolist(), adv_clipped_flat.tolist(), strict=True
                        )
                    ]
                    os.makedirs(token_pair_dir, exist_ok=True)
                    filename = os.path.join(token_pair_dir, f"{self.global_steps}.jsonl")
                    with open(filename, "a") as f:
                        f.write(
                            json.dumps(
                                {
    ...
    def _save_checkpoint(self):
        ...
        torch.save({"sigmoid_entropy_target": self.sigmoid_entropy_target}, trainer_state_path)
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(trainer_state_path):
            ...
            self.sigmoid_entropy_target = trainer_state.get("sigmoid_entropy_target", None)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        if sigmoid_enabled and sigmoid_has_alpha:
                            ...
                            batch.meta_info["sigmoid_entropy_target"] = self.sigmoid_entropy_target
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "avg_token_entropy" in batch.meta_info:
                            reward_extra_infos_dict.setdefault(
                                "avg_token_entropy", batch.meta_info["avg_token_entropy"]
                            )
                        # 可选过滤：当前 batch 内全对/全错的 prompts 不参与本次更新
                        reward_tensor, response_masks = self._filter_all_correct_wrong_samples(
                    ...
                    rollout_rank_log_freq = int(self.config.trainer.get("rollout_rank_log_freq", 10) or 0)
                    rollout_rank_enable = bool(self.config.trainer.get("rollout_rank_enable", False))
                    token_pair_enable_cfg = bool(self.config.trainer.get("cliped_token_pair_enable", False))
                    token_pair_dir_cfg = self.config.trainer.get("cliped_token_pair_dir", None)
                    token_pair_log_freq = int(self.config.trainer.get("cliped_token_pair_log_freq", 10) or 0)
                    clip_enabled_cfg = getattr(
                        getattr(self.config.actor_rollout_ref.actor.policy_loss, "advantage_clip", None),
                        "enable",
                        False,
                    )
                    if token_pair_dir_cfg is None and rollout_data_dir:
                        # 默认与 sequence dump 同目录
                        token_pair_dir_cfg = rollout_data_dir
                    enable_token_pairs_now = (
                        token_pair_enable_cfg
                        and clip_enabled_cfg
                        and token_pair_dir_cfg is not None
                        and (token_pair_log_freq <= 0 or self.global_steps % token_pair_log_freq == 0)
                    )
                    if rollout_data_dir and (rollout_rank_log_freq <= 0 or self.global_steps % rollout_rank_log_freq == 0):
                        self._log_rollout_data(
                            batch,
                            reward_extra_infos_dict,
                            timing_raw,
                            rollout_data_dir,
                            enable_token_ranks=rollout_rank_enable,
                            enable_token_pairs=enable_token_pairs_now,
                            token_pair_dir=token_pair_dir_cfg,
                            token_pair_log_freq=token_pair_log_freq,
                        )
