        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # 统一写入裁剪统计，避免各卡/各 micro-batch list 长度不一致导致 reduce_metrics 报错
                    if clip_metrics is None:
                        clip_metrics = {
                            "adv_clipfrac_pos": torch.tensor(0.0, device=advantages.device),
                            "adv_clipfrac_neg": torch.tensor(0.0, device=advantages.device),
                            "adv_clip_available": torch.tensor(0.0, device=advantages.device),
                            "adv_sigmoid_p0": torch.tensor(float("nan"), device=advantages.device),
                            "adv_sigmoid_available": torch.tensor(0.0, device=advantages.device),
                        }
                        if sigmoid_alpha_pos_cfg is not None:
                        ...
                        clip_metrics = {
                            **clip_metrics,
                            "adv_clip_available": torch.tensor(1.0, device=advantages.device),
                        }
                        # 若上游未返回 p0（如 valid_mask 为空），补 NaN 占位并标记 available=0，便于 nanmean 聚合
                        clip_metrics.setdefault("adv_sigmoid_p0", torch.tensor(float("nan"), device=advantages.device))
                        clip_metrics.setdefault("adv_sigmoid_available", torch.tensor(0.0, device=advantages.device))
                        if sigmoid_alpha_pos_cfg is not None:
                            clip_metrics.setdefault(
                                "adv_sigmoid_alpha_pos", torch.tensor(0.0, device=advantages.device)
                            )
                        if sigmoid_alpha_neg_cfg is not None:
                            clip_metrics.setdefault(
