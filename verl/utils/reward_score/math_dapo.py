...
def _to_ascii_digits(text: str) -> str:
    """将 Unicode 数字符号转换为 ASCII 数字，无法转换则原样保留"""
    converted = []
    for ch in text:
        try:
            converted.append(str(unicodedata.digit(ch)))
...
def normalize_final_answer(final_answer: str) -> str:
    ...
    # Normalize numbers（移除千分位与前导 0）
    # 处理数字中的逗号与简单上标
    normalized = final_answer.replace(",", "")
    superscript_map = str.maketrans({"¹": "1", "²": "2", "³": "3"})
    normalized = normalized.translate(superscript_map)
    normalized = _to_ascii_digits(normalized).strip()

    def _is_int_string(val: str) -> bool:
        return val.isdigit() or (val.startswith("-") and val[1:].isdigit())

    if _is_int_string(normalized):
        try:
