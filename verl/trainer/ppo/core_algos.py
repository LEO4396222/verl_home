...
__all__ = ["register_adv_est", "get_adv_estimator_fn", "AdvantageEstimator", "apply_advantage_clip"]
...
def _apply_coef_clip(
    adv: torch.Tensor,
    log_prob: torch.Tensor,
    mask: torch.Tensor,
    clip_conf: Any,
    rollout_n_val: float | None,
    prob_eps: float,
    need_metrics: bool,
) -> torch.Tensor | tuple[torch.Tensor, dict[str, torch.Tensor] | None]:
    """现有 coef 模式：用 positive_coef / negative_coef 按概率相关的上下界裁剪优势。"""
    positive_prob_threshold = _cfg_get(clip_conf, "positive_prob_threshold")
    negative_prob_threshold = _cfg_get(clip_conf, "negative_prob_threshold")
    positive_coef = _cfg_get(clip_conf, "positive_coef")
    negative_coef = _cfg_get(clip_conf, "negative_coef")
    ...
    if positive_prob_threshold is not None:
        ...
        if rollout_n_val is None or rollout_n_val <= 1:
            warnings.warn(
            ...
            span = (float(rollout_n_val) - 1.0) / math.sqrt(float(rollout_n_val))
    ...
    if negative_prob_threshold is not None:
        ...
        if rollout_n_val is None or rollout_n_val <= 1:
            ...
            span = (1.0 - float(rollout_n_val)) / math.sqrt(float(rollout_n_val))
            denom = max(negative_prob_threshold, prob_eps)
    ...
    with torch.no_grad():
        ...
        output = adv.clone()
        output.masked_scatter_(valid_mask, clipped_valid_advantages)
...
def _apply_quantile_clip(
    ...
    """quantile 模式：基于 batch 内概率/优势分位数裁剪贪心和过度惩罚的 token，抑制熵快速下降。"""
    valid_mask = mask.to(dtype=torch.bool)
    if not torch.any(valid_mask):
        return (adv, None) if need_metrics else adv
    ...
    prob_high_q = _cfg_get(clip_conf, "prob_high_quantile", 0.8)
    prob_low_q = _cfg_get(clip_conf, "prob_low_quantile", 0.2)
    adv_high_q = _cfg_get(clip_conf, "adv_high_quantile", 0.8)
    adv_low_q = _cfg_get(clip_conf, "adv_low_quantile", 0.2)
    for name, q in [
        ("prob_high_quantile", prob_high_q),
        ("prob_low_quantile", prob_low_q),
        ("adv_high_quantile", adv_high_q),
        ("adv_low_quantile", adv_low_q),
    ]:
        if not 0.0 <= float(q) <= 1.0:
            raise ValueError(f"advantage_clip.{name} must be in [0, 1]")

    # 分别在正/负优势子集上计算概率和优势分位数，避免正负混排导致阈值偏移。
    def _quantile_or_none(x: torch.Tensor, q: float):
        return torch.quantile(x, q) if x.numel() > 0 else None

    pos_mask = valid_mask & (adv > 0)
    neg_mask = valid_mask & (adv < 0)

    P_high = _quantile_or_none(token_prob.masked_select(pos_mask), prob_high_q)
    A_high = _quantile_or_none(adv.masked_select(pos_mask), adv_high_q)
    P_low = _quantile_or_none(token_prob.masked_select(neg_mask), prob_low_q)
    A_low = _quantile_or_none(adv.masked_select(neg_mask), adv_low_q)

    enable_clip_high = bool(_cfg_get(clip_conf, "enable_clip_high", True))
    enable_clip_low = bool(_cfg_get(clip_conf, "enable_clip_low", True))
    high_clip_scale = _cfg_get(clip_conf, "high_clip_scale", 1.0)
    low_clip_scale = _cfg_get(clip_conf, "low_clip_scale", 1.0)

    output = adv
    high_mask = torch.zeros_like(valid_mask)
    low_mask = torch.zeros_like(valid_mask)
    if enable_clip_high and P_high is not None and A_high is not None:
        # Clip High: 高概率且高正优势的 token，截到 high_clip_scale * A_high
        high_mask = valid_mask & (token_prob > P_high) & (adv > A_high) & (adv > 0)
        # 约束高侧裁剪不改变优势符号：目标值最小为 0
        clipped_high = adv.new_tensor(high_clip_scale) * A_high
        clipped_high = torch.maximum(clipped_high, adv.new_zeros(()))
        output = torch.where(high_mask, clipped_high, output)
    if enable_clip_low and P_low is not None and A_low is not None:
        # Clip Low: 低概率且强负优势的 token，截到 low_clip_scale * A_low
        low_mask = valid_mask & (token_prob < P_low) & (adv < A_low) & (adv < 0)
        # 约束低侧裁剪不改变优势符号：目标值最大为 0
        clipped_low = adv.new_tensor(low_clip_scale) * A_low
        clipped_low = torch.minimum(clipped_low, adv.new_zeros(()))
        output = torch.where(low_mask, clipped_low, output)

    clip_stats: dict[str, torch.Tensor] | None = None
    if need_metrics:
        valid_tokens = valid_mask.sum().clamp(min=1).to(dtype=adv.dtype)
        clip_stats = {
            "adv_clipfrac_pos": high_mask.float().sum().to(dtype=adv.dtype) / valid_tokens,
            "adv_clipfrac_neg": low_mask.float().sum().to(dtype=adv.dtype) / valid_tokens,
        }
...
def _apply_sigmoid_clip(
    ...
    prob_eps: float,
    need_metrics: bool,
    entropy_current: float | None,
    entropy_target: float | None,
) -> torch.Tensor | tuple[torch.Tensor, dict[str, torch.Tensor] | None]:
    """sigmoid 模式：基于 p0 的正负优势平滑重加权，支持固定概率或概率分位数。"""
    valid_mask = mask.to(dtype=torch.bool)
    if not torch.any(valid_mask):
        return (adv, None) if need_metrics else adv
    ...
    p0_cfg = _cfg_get(clip_conf, "sigmoid_p0_prob", 0.3)
    ...
    if p0_cfg is not None:
        p0 = adv.new_tensor(float(p0_cfg))
    else:
        valid_probs = token_prob[valid_mask]
        p0 = torch.quantile(valid_probs, float(p0_quantile_cfg))
    p0 = p0.clamp(min=prob_eps, max=1.0)
    entropy_target_cfg = _cfg_get(clip_conf, "sigmoid_entropy_target", None)
    entropy_target_val = _to_float(entropy_target) if entropy_target is not None else _to_float(entropy_target_cfg)
    entropy_current_val = _to_float(entropy_current)
    if entropy_current_val is not None and entropy_target_val is not None:
        entropy_current_t = adv.new_tensor(entropy_current_val)
        entropy_target_t = adv.new_tensor(entropy_target_val)
        delta_pos = torch.clamp(entropy_target_t - entropy_current_t, min=0.0)
        delta_neg = torch.clamp(entropy_current_t - entropy_target_t, max=0.0)
    else:
        delta_pos = adv.new_tensor(1.0)
        delta_neg = adv.new_tensor(-1.0)

    pos_mask = valid_mask & (adv > 0) if alpha_pos is not None else None
    neg_mask = valid_mask & (adv < 0) if alpha_neg is not None else None

    weights = adv.new_ones(adv.shape)
    alpha_pos_actual = None
    alpha_neg_actual = None
    if alpha_pos is not None:
        alpha_pos_actual = delta_pos * adv.new_tensor(float(alpha_pos))
    if alpha_neg is not None:
        alpha_neg_actual = delta_neg * adv.new_tensor(float(alpha_neg))

    if alpha_pos_actual is not None and pos_mask is not None and torch.any(pos_mask):
        f_plus = 2.0 / (1.0 + torch.exp(alpha_pos_actual * (token_prob - p0)))
    ...
    if alpha_neg_actual is not None and neg_mask is not None and torch.any(neg_mask):
        f_minus = 2.0 / (1.0 + torch.exp(alpha_neg_actual * (token_prob - p0)))
    ...
    if need_metrics:
        ...
        clip_stats = {
            ...
            "adv_sigmoid_p0": p0.detach(),
            "adv_sigmoid_available": available,
            "adv_sigmoid_amp_frac": amp_frac,
            "adv_sigmoid_shrink_frac": shrink_frac,
        }
        if alpha_pos_actual is not None:
            clip_stats["adv_sigmoid_alpha_pos"] = alpha_pos_actual.detach()
        if alpha_neg_actual is not None:
            clip_stats["adv_sigmoid_alpha_neg"] = alpha_neg_actual.detach()
        return output, clip_stats
    return output
...
def _apply_positional_weight(
    ...
    """按回复内位置对优势加权：前重后轻，中心附近约为 1。"""
    valid_mask = mask.to(dtype=torch.bool)
    if not torch.any(valid_mask):
        return adv

    # 1-based 累积得到每个 token 在有效序列中的位置；无效位置保持 0
    pos_idx = torch.cumsum(valid_mask.int(), dim=1) - 1
    total = valid_mask.sum(dim=1, keepdim=True)

    # 归一化位置到 [0, 1]；长度为 1 时居中 0.5
    denom = torch.clamp(total - 1, min=1)
    pos_norm = pos_idx.to(dtype=adv.dtype) / denom.to(dtype=adv.dtype)
    pos_norm = torch.where(total > 1, pos_norm, adv.new_tensor(0.5))

    weights = 2.0 / (1.0 + torch.exp(adv.new_tensor(alpha) * (pos_norm - 0.5)))
    weights = torch.where(valid_mask, weights, adv.new_ones(adv.shape))
    return adv * weights


def apply_advantage_clip(
    advantages: torch.Tensor,
    old_log_prob: torch.Tensor,
    response_mask: torch.Tensor,
    clip_cfg: Any,
    *,
    rollout_n: float | None = None,
    ...
    # 可选的基于位置的优势加权（与概率裁剪解耦）
    pos_weight_enable = bool(_cfg_get(clip_cfg, "pos_weight_enable", False))
    pos_weight_alpha = _cfg_get(clip_cfg, "pos_weight_alpha", None)
    if pos_weight_enable and pos_weight_alpha is not None and float(pos_weight_alpha) > 0.0:
        advantages = _apply_positional_weight(advantages, response_mask, float(pos_weight_alpha))

    prob_epsilon = _cfg_get(clip_cfg, "prob_epsilon", 1e-6)
    mode = _cfg_get(clip_cfg, "mode", "coef")

    rollout_n_val = rollout_n
    if isinstance(rollout_n_val, torch.Tensor):
        rollout_n_val = rollout_n_val.item()
    rollout_n_val = float(rollout_n_val) if rollout_n_val is not None else None

    if mode == "sigmoid":
        return _apply_sigmoid_clip(
            advantages,
            old_log_prob,
            response_mask,
            clip_cfg,
    ...
    if mode != "coef":
        raise ValueError(f"Unsupported advantage_clip.mode: {mode}")
    return _apply_coef_clip(
        advantages,
        old_log_prob,
        response_mask,
        clip_cfg,
...
def compute_policy_loss_gspo(
    ...
    advantages = _maybe_clip_advantages(
        advantages,
        old_log_prob,
        response_mask,
        config,
