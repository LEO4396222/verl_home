...
class AdvantageClipConfig(BaseConfig):
    ...
    Args:
        ...
        mode (str): Clip mode. "coef" 使用概率系数裁剪；"quantile" 使用分位数裁剪（正/负优势分别计算分位数）。
        prob_high_quantile (float): 高概率分位数（P_high），用于 quantile 模式。
        prob_low_quantile (float): 低概率分位数（P_low），用于 quantile 模式。
        adv_high_quantile (float): 高优势分位数（A_high），用于 quantile 模式。
        adv_low_quantile (float): 低优势分位数（A_low），用于 quantile 模式。
        high_clip_scale (float): High 裁剪时优势缩放系数，乘以 A_high。
        low_clip_scale (float): Low 裁剪时优势缩放系数，乘以 A_low。
        enable_clip_high (bool): 是否启用高概率正优势裁剪分支。
        enable_clip_low (bool): 是否启用低概率负优势裁剪分支。
        sigmoid_p0_prob (float): Sigmoid 模式下参考概率 p0（直接概率阈值）。
        sigmoid_p0_quantile (float | None): Sigmoid 模式下参考概率 p0 的分位数，仅在 sigmoid_p0_prob 为 None 时生效。
    ...
    mode: str = "coef"
    prob_high_quantile: float = 0.8
    prob_low_quantile: float = 0.2
    adv_high_quantile: float = 0.8
    adv_low_quantile: float = 0.2
    high_clip_scale: float = 1.0
    low_clip_scale: float = 1.0
    enable_clip_high: bool = True
    enable_clip_low: bool = True
    sigmoid_p0_prob: float = 0.3
    sigmoid_p0_quantile: float | None = None
